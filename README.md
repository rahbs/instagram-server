...
16	POST	/feed/{feedId}/like	피드 좋아요/취소
17	POST	/comment/{commentId}/like	댓글 좋아요 누르기/취소
18	POST	/comment	댓글 업로드
19	DELETE	/comment/{commentId}	댓글 삭제
20	POST	/followRequest	팔로우 요청 보내기
21	GET	/user/{userIdx}/follow?follow= &limitStart= &limitCount =	팔로워/팔로잉 목록 조회
22	POST	/followAccept/{followRequestId}	팔로우요청 수락
23	DELETE	/followAccept/{followRequestId}	(요청받은사람이)팔로우요청 삭제
24	POST	/user/{userIdx}/closeFriend	친한 친구 리스트에 추가
25	POST	/user/{userIdx}/mute?kind=	해당 유저의 게시물 혹은 스토리 숨기기
26	DELETE	/user/{userIdx}/following	팔로우 취소
27	DELETE	/user/{userIdx}/follower	팔로워 삭제
28	POST	/user/{userIdx}/block	차단
29	GET	/users/notFollowingUser	맞팔로우하지 않은 계정 목록 조회
30	GET	/users?userId=	계정 검색
31	GET	/activities?limitStart= &limitCount=	활동조회
32	POST	/comment/recomment	대댓글생성
